...
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct ProcedureCandidate { pub id: String, pub title: String, pub when_to_use: String, pub steps: Vec<String>, pub verification_steps: Vec<String>, pub source_record_ids: Vec<String>, pub evidence_reference_count: usize, pub confidence: f32, pub review_required: bool, pub mutation_performed: bool }
...
#[derive(Debug, Clone, Serialize, Deserialize)] pub struct FailureAnalysisSummary { pub rejected_patch_count: usize, pub stale_deferred_patch_count: usize, pub warning_event_count: usize }
...
pub fn analyze_failure_patterns(patches: &[Patch], events: &[StoreEvent], namespace: &str, stale_deferred_days: i64, generated_at: DateTime<Utc>) -> FailureAnalysisReport {
    ...
    let warnings: Vec<_> = events.iter().filter(|event| event.namespace == namespace && event.severity != "info").filter_map(|event| event.object_id.clone()).collect();
    ...
    if events.iter().any(|event| event.namespace == namespace && event.severity != "info") { patterns.push(FailurePattern { id: "failure:warning_events".into(), category: "warning_events".into(), affected_object_ids: warnings, recommendation: "Inspect warning and error events for recurring operational causes.".into(), review_required: true }); }
    FailureAnalysisReport { schema_version: 1, generated_at, namespace: namespace.into(), summary: FailureAnalysisSummary { rejected_patch_count: rejected.len(), stale_deferred_patch_count: deferred.len(), warning_event_count: events.iter().filter(|event| event.namespace == namespace && event.severity != "info").count() }, patterns, mutation_performed: false }
}
