...
pub fn build_store_quality(memory: &MemoryQualityReport, relationships: &RelationshipQualityReport, recall: Option<&RecallEffectivenessReport>, pending_patches: usize, runtime_warnings: usize, generated_at: DateTime<Utc>) -> StoreQualityReport {
    let governance_signals = memory.items.iter().filter(|item| item.signals.iter().any(|signal| signal == "missing_evidence" || signal == "contested")).count();
    let mut metrics = vec![
        ...
        StoreQualityMetric { id: "governance".into(), score: Some(100u32.saturating_sub((governance_signals as u32).saturating_mul(15))), signals: if governance_signals > 0 { vec!["governance_review_required".into()] } else { Vec::new() } },
