...
use pi_governance_core::{
    Durability, EvidenceKind, EvidenceRef, MemoryKind, MemoryLayer, RecallEvent, RecallEventClient,
    RecallEventOperation, RecallEventOutcome, RecordClass, RecordStatus, RetrievalFormat,
    RetrievalOptions, RuleType, Scope, SourceKind, StoreEvent, TrustClass,
};
...
pub fn score_memory_worth(
    ...
    let inferred_trust = trust_class.unwrap_or_else(|| {
        if l.contains("don't") || l.contains("do not") || l.contains("not ") {
    ...
    if is_secret_like(observation) {
        ...
        return MemoryWorthReport {
            decision: MemoryWorthDecision::Reject,
            confidence: 0.99,
            suggested_layer: MemoryLayer::L2Playbook,
            suggested_class: RecordClass::Observation,
            suggested_memory_kind: MemoryKind::Fact,
            suggested_rule_type: None,
            suggested_tags: tags,
            durability: Durability::Unknown,
            trust_class: inferred_trust,
            source_kind: inferred_source,
            reasons: vec!["secret-like content".to_string()],
            warnings,
        };
    }
    if ["thanks", "ok", "hello", "nice"].contains(&l.trim()) || l.trim().len() < 8 {
        return MemoryWorthReport {
            decision: MemoryWorthDecision::Reject,
            confidence: 0.9,
            suggested_layer: MemoryLayer::L3Session,
            suggested_class: RecordClass::Observation,
            suggested_memory_kind: MemoryKind::Event,
            suggested_rule_type: None,
            suggested_tags: tags,
            durability: Durability::Temporary,
            trust_class: inferred_trust,
            source_kind: inferred_source,
            reasons: vec!["trivial or non-durable text".to_string()],
            warnings,
        };
    }

    let correction = l.contains("don't")
        || l.contains("do not")
        || l.starts_with("not ")
        || l.contains(" instead")
        || l.contains("not ") && l.contains(",");
    let durable = [
        "always",
        "from now on",
        "going forward",
    ...
    .iter()
    .any(|p| l.contains(p));
    let temporary = l.contains("today")
        || l.contains("temporary")
        || l.contains("for this task")
        || l.contains("right now");
    ...
    if l.contains("retrieval") {
        tags.push("retrieval".to_string());
    }
    if l.contains("note") || l.contains("sed") {
    ...
    if temporary {
        ...
        return MemoryWorthReport {
            decision: MemoryWorthDecision::DailyOnly,
            confidence: 0.7,
            suggested_layer: MemoryLayer::L3Session,
            suggested_class: RecordClass::Observation,
            suggested_memory_kind: MemoryKind::Event,
    ...
    if durable || correction {
        ...
        let rule = if correction {
            Some(RuleType::Correction)
        } else if l.contains("test") || l.contains("release") {
            Some(RuleType::Workflow)
        } else if l.contains("prefer") {
            Some(RuleType::Preference)
        } else {
            Some(RuleType::Convention)
        };
        return MemoryWorthReport {
            decision: MemoryWorthDecision::Candidate,
            confidence: 0.82,
            suggested_layer: MemoryLayer::L2Playbook,
            suggested_class: if correction {
                RecordClass::Correction
            } else {
                RecordClass::Workflow
            },
            suggested_memory_kind: MemoryKind::Instruction,
            suggested_rule_type: rule,
            suggested_tags: tags,
    ...
    MemoryWorthReport {
        decision: MemoryWorthDecision::Inquiry,
        confidence: 0.45,
        suggested_layer: MemoryLayer::L2Playbook,
        suggested_class: RecordClass::Observation,
        suggested_memory_kind: MemoryKind::Fact,
        ...
        trust_class: inferred_trust,
        source_kind: inferred_source,
        reasons: vec!["ambiguous durable intent".to_string()],
        warnings,
    }
}
...
pub fn verify_candidate(
    ...
    if claim.trim().is_empty() {
        checks.push(VerificationCheck {
            name: "non_empty_claim".to_string(),
            status: "fail".to_string(),
            message: Some("claim is empty".to_string()),
        });
        rejected = true;
    } else {
        checks.push(VerificationCheck {
    ...
    if is_secret_like(claim) {
        checks.push(VerificationCheck {
            name: "not_secret_like".to_string(),
            status: "fail".to_string(),
            message: Some("secret-like content".to_string()),
        });
        rejected = true;
    } else {
        checks.push(VerificationCheck {
    ...
    if layer == MemoryLayer::L1Identity {
        requires_review = true;
        checks.push(VerificationCheck {
            name: "l1_manual_review".to_string(),
            status: "warn".to_string(),
            message: Some("L1 requires manual review".to_string()),
        });
    }
    match trust_class {
        ...
        | TrustClass::CodebaseAnalysis
        | TrustClass::AgentInference
        | TrustClass::Unknown => {
            requires_review = true;
            warnings.push("low-trust source requires manual review".to_string());
            checks.push(VerificationCheck {
                name: "low_trust_source".to_string(),
                status: "warn".to_string(),
                message: Some("source cannot auto-apply".to_string()),
            });
        }
        _ => checks.push(VerificationCheck {
            name: "trust_class_present".to_string(),
            status: "pass".to_string(),
            message: None,
        }),
    }
    if matches!(durability, Durability::Temporary | Durability::Task)
        && layer != MemoryLayer::L3Session
    {
        requires_review = true;
        warnings.push("temporary/task durability should stay L3".to_string());
    }
    VerificationResult {
        verified: !rejected && !requires_review,
...
pub fn extract_decisions(text: &str) -> Vec<String> {
    ...
    for line in text.lines() {
        ...
        if lower.starts_with("#decision") {
            decisions.push(l.trim_start_matches("#decision").trim().to_string());
        ...
        } else if let Some((_, rest)) = lower.split_once("we decided") {
            decisions.push(rest.trim().to_string());
        }
    }
    if decisions.is_empty() && lower(text).contains("#decision") {
        if let Some((_, rest)) = text.split_once("#decision") {
            decisions.push(rest.trim().to_string());
        }
    }
    decisions
    ...
    Ok(store
        ...
        .filter(|e| {
            e.namespace == namespace
                && e.message.contains("session_entry")
                && lower(&e.message).contains(&q)
                && after.map(|a| e.created_at >= a).unwrap_or(true)
                && project
                    .map(|p| e.message.contains(&format!("\"project\":\"{p}\"")))
                    .unwrap_or(true)
        })
        .collect())
}
...
pub fn session_decisions(
    ...
    for e in store.load_events()? {
        if e.namespace != namespace
            || !e.message.contains("session_entry")
            || after.map(|a| e.created_at < a).unwrap_or(false)
        {
            continue;
        }
...
pub fn build_context(
    ...
    for ranked in &bundle.records {
        let r = &ranked.record;
        if r.status == RecordStatus::Contested {
            contested.push(r.claim.clone());
            if !include_contested {
                continue;
            }
        }
        match r.layer {
            MemoryLayer::L1Identity => l1.push(r.claim.clone()),
            MemoryLayer::L2Playbook => l2.push(r.claim.clone()),
            MemoryLayer::L3Session => {
                if include_l3 {
                    l3.push(r.claim.clone())
                }
            }
        }
    }
...
pub fn recall_xray(
    ...
    let included_ids: std::collections::HashSet<String> =
        bundle.records.iter().map(|r| r.record.id.clone()).collect();
    let included = bundle
        .records
        .iter()
        .map(|r| RecallIncluded {
            record_id: r.record.id.clone(),
            layer: r.record.layer,
            score: r.score,
            matched_terms: r.matched_terms.clone(),
            matched_fields: r.matched_fields.clone(),
            trust_class: r.record.trust_class,
            memory_kind: r.record.memory_kind,
            evidence_state: if r.record.evidence.is_empty() {
                "missing"
            } else {
                "present"
            }
            .to_string(),
            reason: "matched query and filters".to_string(),
        })
        .collect::<Vec<_>>();
    let excluded = records
        .into_iter()
        .filter(|r| r.namespace == namespace && !included_ids.contains(&r.id))
        .map(|r| {
            let reason = if r.status == RecordStatus::Tombstoned {
                "status tombstoned"
            } else if r.status == RecordStatus::Superseded {
                "status superseded"
            } else if r.status == RecordStatus::Contested && !include_contested {
                "contested excluded"
            } else if r.layer == MemoryLayer::L3Session && !include_l3 {
                "L3 excluded by default"
            } else if project
                .as_ref()
                .is_some_and(|p| !r.scope.matches_project_filter(Some(p)))
            {
                "project mismatch"
            } else {
                "no lexical match or budget omitted"
            };
            RecallExcluded {
                record_id: r.id,
    ...
    Ok(RecallXrayReport {
        ...
        budget: RecallBudget {
            requested: budget,
            used: bundle.used_estimated_tokens,
...
#[allow(clippy::too_many_arguments)]
pub fn record_recall_event_with_details(
    store: &JsonlStore,
    namespace: &str,
    client: RecallEventClient,
    operation: RecallEventOperation,
    query: &str,
    selected_record_ids: Vec<String>,
    excluded_reason_counts: std::collections::BTreeMap<String, usize>,
    outcome: Option<RecallEventOutcome>,
    budget_requested: usize,
    budget_used: usize,
) -> Result<bool> {
    let mut event = RecallEvent::new(
        namespace,
        client,
        operation,
        hash_recall_query(query),
        selected_record_ids,
        budget_requested,
        budget_used,
    );
    event.excluded_reason_counts = excluded_reason_counts;
    event.outcome = outcome;
    store.record_recall_event(&event)
}

pub fn recall_exclusion_counts(
    report: &RecallXrayReport,
) -> std::collections::BTreeMap<String, usize> {
    let mut counts = std::collections::BTreeMap::new();
    for item in &report.excluded {
        *counts.entry(item.reason.clone()).or_insert(0) += 1;
    }
    counts
}

pub fn evidence_for_capture(
    source_kind: SourceKind,
    trust_class: TrustClass,
    durability: Durability,
) -> EvidenceRef {
    let mut ev = EvidenceRef::new(
        EvidenceKind::Conversation,
        format!("capture:{}", Utc::now().timestamp_millis()),
    );
...
pub fn parse_days_after(days: Option<i64>) -> Option<DateTime<Utc>> {
...
pub fn scope_for_project(project: Option<String>) -> Scope {
