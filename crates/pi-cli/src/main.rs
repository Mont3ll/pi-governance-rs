...
use pi_governance_core::{
    ContestResolution, Durability, EvidenceKind, EvidenceRef, MemoryKind, MemoryLayer, PatchStatus,
    PolicyProfile, RecallEventClient, RecallEventOperation, RecallEventOutcome, RecordClass,
    RetrievalFormat, RetrievalOptions, RuleType, Scope, SourceKind, StoreEvent, TrustClass,
};
use pi_governance_engine::{
    ...
    claim_from_capture, evidence_for_capture, generate_procedure_candidates, read_text_input,
    ...
    MemoryWorthDecision, MigrationInput, PatchInspection, PatchSummary, ProposalInput,
    ReconcileInput, RecordInspection, ReinforceInput, ResolveContestInput, SupersedeInput,
    TombstoneInput,
};
...
fn audit_check(
    checks: &mut Vec<ReleaseAuditCheck>,
...
fn print_record_inspection(inspection: &RecordInspection) {
    ...
    if let Some(project) = record_project(inspection) {
    ...
    if !record.evidence.is_empty() {
        ...
        for evidence in &record.evidence {
            println!("  - {} ({:?})", evidence.uri, evidence.kind);
        }
    }
    println!("\nCreated:\n  {}", record.created_at);
    println!("Updated:\n  {}", record.updated_at);
    ...
    println!(
        "  Supersedes: {}",
        if inspection.revision.supersedes.is_empty() {
            "none".to_string()
        } else {
            inspection.revision.supersedes.join(", ")
        }
    );
    println!(
        "  Superseded by: {}",
        if inspection.revision.superseded_by.is_empty() {
            "none".to_string()
        } else {
            inspection.revision.superseded_by.join(", ")
        }
    );
    ...
    if !inspection.related_patches.is_empty() {
        println!("\nRelated patches:");
        for patch in &inspection.related_patches {
            println!("  - {}", patch);
        }
    }
    println!("\nNext:");
    ...
    println!(
        "  pi contest {} --reason \"...\" --evidence-uri review:...",
        record.id
    );
    println!(
        "  pi supersede {} --class {:?} --claim \"...\" --reason \"...\" --evidence-uri review:...",
...
fn review_detail_from_inspection(inspection: &PatchInspection) -> ReviewDetail {
    ...
    let evidence = latest
        .map(|p| p.evidence.iter().map(|e| e.uri.clone()).collect())
        .unwrap_or_default();
    let namespace = latest
        .map(|p| p.namespace.clone())
        .unwrap_or_else(|| "unknown".to_string());
    let project = latest
        .and_then(|p| p.proposed_record.as_ref())
        .and_then(|r| r.scope.key.clone());
    ...
    if inspection.can_apply_without_force {
        next_actions.push(format!("pi review {} --apply", inspection.summary.patch_id));
    }
    if inspection.can_apply_with_force {
        next_actions.push(format!(
            "pi review {} --apply --force",
            inspection.summary.patch_id
        ));
    }
    ReviewDetail {
...
fn default_mcp_config_path(client: &str) -> Result<PathBuf> {
    ...
    Ok(match client {
        ...
        other => {
            anyhow::bail!("unsupported mcp client: {other}. Use opencode, codex, or pi-agent.")
        }
    })
}
...
fn install_json_config(
    path: &Path,
    key: &str,
    server: serde_json::Value,
    dry_run: bool,
    no_backup: bool,
) -> Result<(String, Option<PathBuf>, Vec<String>)> {
    let existed = path.exists();
    let mut root = if existed {
        let text = fs::read_to_string(path)?;
        if path.extension().and_then(|s| s.to_str()) == Some("jsonc") && has_jsonc_comments(&text) {
            anyhow::bail!("refusing to rewrite JSONC comments; use mcp-config output as a snippet");
        }
    ...
    let preserved = extract_server_names(&root, key)
        .into_iter()
        .filter(|n| n != "pi-governance")
        .collect::<Vec<_>>();
    let action = if root.get(key).and_then(|v| v.get("pi-governance")).is_some() {
        "update"
    } else {
        "add"
    }
    ...
    root.get_mut(key)
        ...
        .insert("pi-governance".into(), server);
    if dry_run {
        return Ok((action, None, preserved));
    }
    if let Some(parent) = path.parent() {
        fs::create_dir_all(parent)?;
    }
    let backup = if existed && !no_backup {
...
fn install_codex_config(
    ...
    let preserved = text
        ...
        .map(str::to_string)
        .collect::<Vec<_>>();
    let action = if text.contains("[mcp_servers.pi-governance]") {
        "update"
    } else {
        "add"
    }
    ...
    for line in text.lines() {
        if line.trim() == "[mcp_servers.pi-governance]" {
            skipping = true;
            continue;
        }
...
fn confirm_install(yes: bool, dry_run: bool) -> Result<()> {
    ...
    print!("Proceed with MCP config install? [y/N] ");
    io::stdout().flush()?;
    ...
    if matches!(s.trim(), "y" | "Y" | "yes" | "YES") {
        ...
        anyhow::bail!("installation cancelled; rerun with --yes to confirm")
    }
}
...
fn read_server_from_config(client: &str, path: &Path) -> Result<(Option<String>, Vec<String>)> {
    ...
    match client {
        "opencode" => {
            ...
            let arr = v["mcp"]["pi-governance"]["command"].as_array().cloned();
            if let Some(a) = arr {
                Ok((
        ...
        "pi-agent" => {
            let v: serde_json::Value = serde_json::from_str(&text)?;
            let s = &v["mcpServers"]["pi-governance"];
            Ok((
                s["command"].as_str().map(str::to_string),
                s["args"]
                    .as_array()
                    .map(|a| {
                        a.iter()
        ...
        "codex" => {
            ...
            for l in text.lines() {
                let t = l.trim();
                if t == "[mcp_servers.pi-governance]" {
                    in_sec = true;
                    continue;
                }
                ...
                if in_sec && t.starts_with("command") {
                    cmd = t
                        .split_once('=')
                        .map(|(_, v)| v.trim().trim_matches('"').to_string());
                }
                if in_sec && t.starts_with("args") {
                    args = t
                        .split('"')
                        .skip(1)
                        .step_by(2)
...
"#,
    ...
    if !out.status.success() {
        anyhow::bail!("tools/list process failed");
    }
...
fn is_daily_target(target: Option<&str>) -> bool {
    matches!(
        target
            ...
            .as_deref(),
        Some("daily") | Some("session") | Some("l3") | Some("l3-session")
    )
}
...
fn is_long_term_target(target: Option<&str>) -> bool {
    matches!(
        target
            .map(|t| t.to_lowercase().replace('_', "-"))
            .as_deref(),
        Some("long-term") | Some("memory") | Some("l2") | Some("l2-playbook")
    )
}
...
fn print_event_list(events: Vec<StoreEvent>, json_out: bool) -> Result<()> {
    if json_out {
        ...
        for e in events {
            println!("{} {}", e.created_at, e.message);
        }
    }
    Ok(())
}
    ...
    match cli.command {
        ...
        Commands::AgentInstructions { json } => {
            ...
            if json {
                println!(
                    "{}",
                    serde_json::to_string_pretty(&AgentInstructionsReport { instructions })?
                );
            } else {
        ...
        Commands::Review {
            ...
            if let Some(reject_id) = reject {
                let result = engine.reject_patch_by_id(
                    &reject_id,
                    &namespace,
                    &reason.ok_or_else(|| anyhow::anyhow!("--reason is required for --reject"))?,
                )?;
                if json {
                    ...
                    println!(
                        "Patch {}\nStatus: {}\nReason: {}",
                        result.patch_id, result.status, result.reason
                    );
                }
            } else if let Some(defer_id) = defer {
                let result = engine.defer_patch_by_id(
                    &defer_id,
                    &namespace,
                    &reason.ok_or_else(|| anyhow::anyhow!("--reason is required for --defer"))?,
                )?;
                if json {
                    println!("{}", serde_json::to_string_pretty(&result)?);
                } else {
                ...
                if apply {
                    ...
                    if json {
                        ...
                        println!(
                            "Decision: {}",
                            detail.decision.clone().unwrap_or_else(|| "n/a".to_string())
                        );
                        println!("Namespace: {}", detail.namespace);
                        if let Some(project) = &detail.project {
                            println!("Project: {}", project);
                        }
                        if let Some(target) = &detail.target_id {
                        ...
                        if !detail.next_actions.is_empty() {
                            println!("\nNext:");
                ...
                let patches: Vec<ReviewPatch> = engine
                    .list_patches(200)?
                    .into_iter()
                    .filter(|p| p.latest_status == PatchStatus::Proposed)
                    .map(|p| {
                        let mut review = review_patch_from_summary(&p);
                        if let Ok(detail) = engine
                            .inspect_patch(&p.patch_id)
                            .map(|inspection| review_detail_from_inspection(&inspection))
                        {
        ...
        Commands::Demo { json, reset } => {
            let demo_path = if store_path.as_path() == std::path::Path::new(".pi") {
                PathBuf::from("/tmp/pi-governance-demo")
            } else {
                store_path.clone()
            };
            if demo_path.as_path() == std::path::Path::new(".pi") || demo_path.ends_with("target") {
                anyhow::bail!("refusing to use protected demo path");
            }
            if demo_path.exists() {
                let non_empty = std::fs::read_dir(&demo_path)?.next().is_some();
                if non_empty && !reset {
                    anyhow::bail!("demo store exists and is non-empty; pass --reset to replace it");
                }
                if reset {
            ...
            let input = |class, claim: &str, tag: &str, uri: &str| {
                proposal_input(
                    "default".to_string(),
                    class,
                    claim.to_string(),
                    ...
                    vec![tag.to_string()],
                    ev(uri),
                    Some("demo memory".to_string()),
                    None,
                    None,
                    None,
                    TrustClass::DirectUserInstruction,
                    Durability::Project,
                    SourceKind::ManualCli,
                )
            };
            demo_engine.propose_record(
                input(
                    RecordClass::Preference,
                    "Use cargo test --workspace before tagging Rust release candidates.",
                    "release",
                    "demo:preference",
                ),
                true,
            ...
            demo_engine.propose_record(
                input(
                    ...
                    "If MCP smoke tests fail, inspect tools/list before tools/call.",
                    "mcp",
                    "demo:correction",
                ),
                true,
            ...
            let contested = demo_engine
                .propose_record(
                    input(
                        RecordClass::Observation,
                        "Old release note says rc.1 is current, but rc.3 supersedes it.",
                        "contest",
                        "demo:contested",
                    ),
                    true,
                ...
                .record_id
                .unwrap();
            demo_engine.contest_record(
                ContestInput {
                    namespace: "default".to_string(),
                    target_id: contested,
                    evidence_refs: ev("demo:contest"),
                    reason: "demo contested record".to_string(),
                },
                true,
                true,
            )?;
            ...
            demo_engine.supersede_record(
                SupersedeInput {
                    ...
                    evidence_refs: ev("demo:new-current"),
                    reason: "demo supersession".to_string(),
                },
                true,
                true,
            )?;
            let tomb = demo_engine
                .propose_record(
                    input(
                        RecordClass::Workflow,
                        "Old instruction: publish immediately after tests.",
                        "tombstone",
                        "demo:tombstone",
                    ),
                    true,
                ...
                .record_id
                .unwrap();
            demo_engine.tombstone_record(
                TombstoneInput {
                    namespace: "default".to_string(),
                    target_id: tomb,
                    evidence_refs: ev("demo:no-publish"),
                    reason: "this repo does not publish during RC validation".to_string(),
                },
                true,
                true,
            )?;
            ...
            demo_engine.propose_record(
                proposal_input(
                    "default".to_string(),
                    RecordClass::Workflow,
                    "Review pending patches before release.".to_string(),
                    0.75,
                    Scope::project("pi-governance-rs"),
                    vec!["review".to_string()],
                    ev("demo:pending-review"),
                    Some("demo pending patch".to_string()),
                    Some(MemoryLayer::L2Playbook),
                    Some(MemoryKind::Instruction),
                    Some(RuleType::Workflow),
                    TrustClass::DirectUserInstruction,
                    Durability::Project,
                    SourceKind::ManualCli,
                ),
            ...
            let report = DemoReport {
                store: demo_path.display().to_string(),
                records: demo_engine.list_records(200)?.len(),
                pending_patches: demo_engine
                    .list_patches(200)?
                    .iter()
                    .filter(|p| p.latest_status == PatchStatus::Proposed)
                    .count(),
                namespaces: vec![
                    "default".to_string(),
                    "strict-demo".to_string(),
                    "permissive-demo".to_string(),
                ],
                try_commands: vec![
                    format!("pi --store {} review", demo_path.display()),
                    format!(
                        "pi --store {} retrieve \"release workflow\" --explain",
                        demo_path.display()
                    ),
                ],
            };
            if json {
                ...
                println!("PI Demo Store Created\n");
                println!("Store: {}", report.store);
                println!("Records: {}", report.records);
                println!("Pending patches: {}", report.pending_patches);
                println!("Namespaces: {}", report.namespaces.join(", "));
                println!("\nTry:");
                for cmd in &report.try_commands {
                    println!("  {}", cmd);
                }
                println!("  pi --store {} doctor", report.store);
                println!("  pi --store {} namespace doctor", report.store);
                println!("  pi --store {} policy doctor", report.store);
            }
        }
        ...
        Commands::Capture {
            ...
            let source = source_kind.unwrap_or(if stdin {
                SourceKind::Stdin
            } else {
                SourceKind::ManualCli
            });
            if is_daily_target(target.as_deref()) && !is_long_term_target(target.as_deref()) {
                ...
                let report = pi_governance_engine::CaptureReport {
                    input_summary: input_text.chars().take(80).collect(),
                    candidates: Vec::new(),
                    daily_only: vec![input_text.clone()],
                    inquiries: Vec::new(),
                    rejected: Vec::new(),
                    applied: false,
                };
                if json {
                    ...
                    println!(
                        "Captured L3/session entry. Decisions: {}",
                        pi_governance_engine::extract_decisions(&input_text).len()
                    );
                }
            } else {
                    ...
                    MemoryWorthDecision::Candidate => {
                        ...
                        if !dry_run {
                            let result = engine.propose_record(
                                proposal_input(
                                    namespace.clone(),
                                    worth.suggested_class.clone(),
                                    claim.clone(),
                                    worth.confidence,
                                    scope_for_project(project.clone()),
                                    tags,
                                    vec![evidence_for_capture(
                                        source,
                                        worth.trust_class,
                                        worth.durability,
                                    )],
                                    Some("captured deterministic memory candidate".to_string()),
                                    Some(suggested_layer),
                                    Some(worth.suggested_memory_kind),
                                    worth.suggested_rule_type,
                                    worth.trust_class,
                                    worth.durability,
                                    source,
                                ),
                ...
                if json {
                    ...
                    println!("No durable candidate created.");
                } else {
                    for c in &report.candidates {
                        println!(
                            "candidate {} layer={} trust_class={:?}",
                            c.patch_id.as_deref().unwrap_or("dry-run"),
                            c.suggested_layer,
                            c.trust_class
                        );
                    }
        ...
        Commands::InspectPatch { patch_id, json } => {
            ...
            if json {
                ...
                println!(
                    "Can apply without force: {}",
                    inspection.can_apply_without_force
                );
        ...
        Commands::PrivacyPurge {
            ...
            if apply {
                let fingerprint = fingerprint.context(
                    "privacy purge apply requires --fingerprint from a reviewed preview",
                )?;
                let result = engine.apply_privacy_purge(
                    &namespace,
                    &target_id,
                    &reason,
                    &correlated_patch_ids,
                ...
                if json {
                    ...
                    println!(
                        "Privacy purge preview for {} (fingerprint {})",
        ...
        Commands::ProcedureCandidates {
            min_source_records,
            json,
        } => {
            let report = generate_procedure_candidates(
                &store.load_records()?,
                &namespace,
                min_source_records,
                chrono::Utc::now(),
        ...
        Commands::FailureAnalysis { stale_days, json } => {
            ...
            if json {
                ...
                println!(
                    "PI Failure Analysis\nPatterns: {}\nReview required. No mutation performed.",
                    report.patterns.len()
                );
            }
        }
        ...
        Commands::Graph {
            ...
            if max_nodes == 0 || max_edges == 0 {
                anyhow::bail!("graph limits must be greater than zero");
            }
        ...
        Commands::Quality { command } => match command {
            QualityCommands::Memory { json } => {
                let report =
                    analyze_memory_quality(&store.load_records()?, &namespace, chrono::Utc::now());
                if json {
                    println!("{}", serde_json::to_string_pretty(&report)?);
                } else {
                    println!(
                        "PI Memory Quality\nAverage: {}/100\nLow quality: {}",
                        report.summary.average_quality, report.summary.low_quality_count
                    );
                }
            }
            QualityCommands::Relationship { json } => {
                ...
                if json {
                    ...
                    println!(
                        "PI Relationship Quality\nAverage: {}/100\nDangling: {}",
                        report.summary.average_relationship_quality,
                        report.summary.dangling_edge_count
                    );
                }
            }
            QualityCommands::Recall { json } => {
                let report = analyze_recall_effectiveness(
                    &store.load_records()?,
                    &store.load_recall_events()?,
                    &namespace,
                    chrono::Utc::now(),
                );
                if json {
                    ...
                    println!(
                        "PI Recall Effectiveness\nAverage: {}/100\nEvents: {}",
                        report.summary.average_effectiveness, report.summary.total_events
                    );
                }
            }
        ...
        Commands::Config { command } => match command {
            ...
            ConfigCommands::SetRecallTelemetry {
                enabled,
                max_events,
            } => {
                if max_events == 0 {
                ...
                config.recall_telemetry.enabled = match enabled.as_str() {
                    "true" | "on" | "enabled" => true,
                    "false" | "off" | "disabled" => false,
                    _ => anyhow::bail!("enabled must be true/on or false/off"),
                };
                config.recall_telemetry.max_events = max_events;
                store.save_config(&config)?;
                println!(
                    "{}",
                    serde_json::to_string_pretty(&config.recall_telemetry)?
                );
            }
        },
        ...
        Commands::Namespace { command } => match command {
            ...
            NamespaceCommands::Doctor { json } => {
                ...
                if json {
                    ...
                    println!(
                        "Records without explicit namespace: {}",
                        report.records_without_explicit_namespace
                    );
                    println!(
                        "Cross-namespace duplicate IDs: {}",
                        report.cross_namespace_duplicate_ids
                    );
                }
            }
        },
                ...
                Some(path) => {
                    ...
                    println!(
                        ...
                        bundle
                            .namespace
                            .as_deref()
                            .unwrap_or(if bundle.all_namespaces {
                                "<all>"
                            } else {
                                "<none>"
                            })
        ...
        Commands::Import {
            ...
            let report = engine.import_store_from_path(
                &path,
                ImportInput {
                    namespace: namespace.clone(),
                    preserve_namespaces,
        ...
        Commands::Reconcile {
            ...
            let destination = engine
                .store()
                .read_portable_bundle_from_path(&path)
            ...
            if json {
                ...
                if let Some(records) = report.sections.get("records") {
                    ...
                    println!(
                        "Destination-only records: {}",
                        records.destination_only_ids.len()
                    );
        ...
        Commands::StoreIntegrity {
            ...
            if apply {
                let fingerprint = fingerprint
                    .context("--apply requires --fingerprint from a reviewed preview")?;
                ...
                if json {
                    ...
                    if report.migration_needed {
                        println!(
                            "Next: pi --store <path> store-integrity --apply --fingerprint {}",
                            report.fingerprint
                        );
                    }
                }
        ...
        Commands::Doctor { json } => {
            ...
            if json {
                ...
                println!(
                    "Records in current namespace: {}",
                    report.records_in_current_namespace
                );
                println!(
                    "Active in current namespace: {}",
                    report.active_in_current_namespace
                );
                println!(
                    "Contested in current namespace: {}",
                    report.contested_in_current_namespace
                );
                println!(
                    "Superseded in current namespace: {}",
                    report.superseded_in_current_namespace
                );
                println!(
                    "Tombstoned in current namespace: {}",
                    report.tombstoned_in_current_namespace
                );
                ...
                println!(
                    "Latest proposed patches: {}",
                    report.proposed_patches_latest
                );
                println!("Latest applied patches: {}", report.applied_patches_latest);
                println!(
                    "Latest rejected patches: {}",
                    report.rejected_patches_latest
                );
        ...
        Commands::List { limit, layer } => {
            ...
            for record in records
                .into_iter()
                .filter(|record| layer.map(|l| record.layer == l).unwrap_or(true))
            {
                println!(
        ...
        Commands::InspectRecord {
            ...
            Some(inspection) => {
                if let Some(layer) = layer {
                    if inspection.record.layer != layer {
                        anyhow::bail!("record layer does not match filter");
                    }
                }
                if json {
                    println!("{}", serde_json::to_string_pretty(&inspection)?);
                } else {
                    print_record_inspection(&inspection);
                }
            }
            None => {
                if json {
                    println!(
                        "{}",
                        serde_json::to_string_pretty(&InspectRecordError {
                            error: "record_not_found".to_string(),
        ...
        Commands::Inbox {
            ...
            if let Some(id) = apply {
                ...
                if json {
                    ...
                    println!("Patch {} rejected", result.patch_id);
                }
                ...
                if json {
                    ...
                    println!("Patch {} deferred", result.patch_id);
                }
            } else {
                ...
                for p in engine.list_patches(200)? {
                    ...
                    let proposed = inspection
                        .history
                        .last()
                        .and_then(|h| h.proposed_record.as_ref());
                    if let Some(layer_filter) = layer {
                        if proposed.map(|r| r.layer) != Some(layer_filter) {
                            continue;
                        }
                    }
                ...
                if json {
                    println!(
                        ...
                        serde_json::to_string_pretty(
                            &json!({"pending_count":rows.len(),"patches":rows})
                        )?
                    );
                } else {
                    println!("PI Candidate Inbox\n");
                    for row in rows {
                        println!(
                            "{} status={} layer={} claim={}",
                            row["patch_id"].as_str().unwrap_or(""),
                            row["status"],
                            row["layer"],
        ...
        Commands::Session { command } => match command {
            SessionCommands::Add {
                ...
                if json {
                    ...
                    println!("Session entry: {}", event.id);
                }
            }
        ...
        Commands::RecallXray {
            ...
            if json {
                ...
                println!("Recall X-ray: {}", report.query);
                for r in report.included {
                    println!(
                        "included {} layer={} score={:.3}",
                        r.record_id, r.layer, r.score
                    );
                }
                for r in report.excluded {
                    println!("excluded {} reason={}", r.record_id, r.reason);
                }
            }
        }

        Commands::RecallFeedback {
            outcome,
            record_ids,
        } => {
            let outcome = match outcome.as_str() {
                "successful" | "success" => RecallEventOutcome::Successful,
                "corrected" | "correction" => RecallEventOutcome::Corrected,
                "ignored" => RecallEventOutcome::Ignored,
                _ => anyhow::bail!("outcome must be successful, corrected, or ignored"),
            };
            ...
            println!(
                "{}",
                serde_json::to_string_pretty(&json!({"recorded":recorded}))?
            );
        }
        ...
        Commands::McpInstall {
            ...
            let (action, backup, preserved) = match client.as_str() {
                "pi-agent" => install_json_config(
                    &config_path,
                    "mcpServers",
                    shared_mcp_json(&command_path, &store_path, &namespace)["mcpServers"]
                        ["pi-governance"]
                        .clone(),
                    dry_run,
                    no_backup,
                )?,
                "opencode" => install_json_config(
                    &config_path,
                    "mcp",
                    opencode_mcp_json(&command_path, &store_path, &namespace)["mcp"]
                        ["pi-governance"]
                        .clone(),
                    dry_run,
                    no_backup,
                ...
                other => anyhow::bail!(
                    "unsupported mcp client: {other}. Use opencode, codex, or pi-agent."
                ),
            };
        ...
        Commands::McpDoctor {
            ...
            if config_exists {
                match read_server_from_config(&client, &config_path) {
                    Ok((c, a)) => {
                        parse_ok = true;
                        configured_command = c;
                        args = a;
                    }
            ...
            let command_exists = configured_command
                .as_ref()
                .map(|c| Path::new(c).exists())
                .unwrap_or(false);
            let command_executable = configured_command
                .as_ref()
                .and_then(|c| fs::metadata(c).ok())
                .map(|m| m.permissions().mode() & 0o111 != 0)
                .unwrap_or(false);
            let store_exists = store_path.exists();
            let namespace_matches = args
                .windows(2)
                .any(|w| w[0] == "--namespace" && w[1] == namespace);
            let store_matches = args
                .windows(2)
                .any(|w| w[0] == "--store" && w[1] == store_path.display().to_string());
            ...
            let tools = if let Some(c) = &configured_command {
                run_tools_list(c, &args).unwrap_or_default()
            } else {
                Vec::new()
            };
            for e in expected {
                tool_map.insert(e, tools.iter().any(|t| t == e));
            }
            ...
            if json {
                println!(
                    "{}",
                    serde_json::to_string_pretty(
                        &serde_json::json!({"client":client,"config_path":config_path,"checks":checks,"tools":tool_map,"result": if pass {"pass"} else {"fail"}})
                    )?
                );
            } else {
                println!(
                ...
                println!("\nExpected tools:");
                for (t, p) in &tool_map {
                    println!("  {t}: {}", if *p { "ok" } else { "not available" });
                }
        ...
        Commands::ReleaseAudit { json } => {
            ...
            audit_check(
                ...
                "version",
                true,
                "version command is compiled into CLI",
            );
            audit_check(
                &mut checks,
                &mut failures,
                "doctor-json",
                engine.doctor_in_namespace(&namespace).is_ok(),
                "doctor failed",
            );
            audit_check(
                &mut checks,
                &mut failures,
                "namespace-doctor-json",
                engine.namespace_doctor().is_ok(),
                "namespace doctor failed",
            );
            audit_check(
                &mut checks,
                &mut failures,
                "policy-doctor-json",
                engine.policy_doctor().is_ok(),
                "policy doctor failed",
            );
            audit_check(
                &mut checks,
                &mut failures,
                "smoke-test",
                GovernanceEngine::run_smoke_test().result == "pass",
                "smoke test failed",
            );
            ...
            audit_check(
                ...
                changelog.contains("v1.1.0")
                    ...
                    && changelog.contains("v1.0.0-rc.5")
                    && changelog.contains("v1.0.0-rc.2")
                    && changelog.contains("v1.0.0-rc.1")
                    && changelog.contains("v0.10.1")
                    && changelog.contains("v0.1.0"),
                "changelog missing expected versions",
            );
            let readme = include_str!("../README.md");
            audit_check(
                &mut checks,
                &mut failures,
                "readme-command-matrix",
                [
                    "init",
                    "doctor",
                    "migrate",
                ...
                .iter()
                .all(|cmd| readme.contains(cmd)),
                "README command matrix incomplete",
            );
            ...
            let missing_tools: Vec<_> = required_tools
                .iter()
                .filter(|required| !registered_tools.iter().any(|actual| actual == **required))
                .copied()
                .collect();
            let mcp_tools_detail = if missing_tools.is_empty() {
                "actual MCP registry contains every required tool".to_string()
            } else {
                format!(
                    "actual MCP registry is missing: {}",
                    missing_tools.join(", ")
                )
            };
            audit_check(
            ...
            audit_check(
                ...
                &mut failures,
                "mcp-config",
                mcp_config.to_string().contains("mcp-stdio"),
                "mcp config missing mcp-stdio",
            );
