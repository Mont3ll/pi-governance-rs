...
fn initialize_and_doctor_expose_active_store_identity() {
    ...
    let initialize_first = mcp_call(
        &first,
        "persistent-intelligence",
        serde_json::json!({"jsonrpc":"2.0","id":1,"method":"initialize","params":{}}),
    );
    let initialize_second = mcp_call(
        &second,
        "persistent-intelligence",
        serde_json::json!({"jsonrpc":"2.0","id":2,"method":"initialize","params":{}}),
    );
    assert_eq!(
        initialize_first["result"]["piStoreIdentity"]["resolved"],
        true
    );
    assert_eq!(
        initialize_first["result"]["piStoreIdentity"]["namespace"],
        "persistent-intelligence"
    );
    assert_ne!(
        initialize_first["result"]["piStoreIdentity"]["store"],
        initialize_second["result"]["piStoreIdentity"]["store"]
    );

    let doctor = mcp_call(
        &first,
        "persistent-intelligence",
        serde_json::json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"pi.doctor","arguments":{}}}),
    );
    assert_eq!(
        doctor["result"]["structuredContent"]["piStoreIdentity"],
        initialize_first["result"]["piStoreIdentity"]
    );
}
...
fn list_tools_return_object_shaped_structured_content() {
    ...
    assert!(Command::new(bin())
        .args(["demo", "--store", &store, "--reset"])
        .status()
        .unwrap()
        .success());

    let patches = mcp_call(
        &store,
        "interop-test",
        serde_json::json!({
    ...
    assert!(
        patch_sc.is_object(),
        "structuredContent must be object: {patch_sc}"
    );
    assert!(
        patch_sc["patches"].is_array(),
        "patches must be array: {patch_sc}"
    );
    assert!(
        patch_sc["count"].is_number(),
        "count must be number: {patch_sc}"
    );

    let records = mcp_call(
        &store,
        "interop-test",
        serde_json::json!({
    ...
    assert!(
        record_sc.is_object(),
        "structuredContent must be object: {record_sc}"
    );
    assert!(
        record_sc["records"].is_array(),
        "records must be array: {record_sc}"
    );
    assert!(
        record_sc["count"].is_number(),
        "count must be number: {record_sc}"
    );
}
...
fn mcp_default_namespace_is_server_namespace() {
    ...
    let interop_review = Command::new(bin())
        .args(["--store", &store, "--namespace", "interop-test", "review"])
        .output()
        .unwrap();
    assert!(interop_review.status.success());
    let interop_text = String::from_utf8(interop_review.stdout).unwrap();
    assert!(
        interop_text.contains("rc7 namespace propagation test memory"),
        "{interop_text}"
    );

    let default_patches = mcp_call(
        &store,
        "default",
        serde_json::json!({
    ...
    let default_patch_text = default_patches["result"]["content"][0]["text"]
        .as_str()
        .unwrap();
    assert!(
        !default_patch_text.contains("rc7 namespace propagation test memory"),
        "{default_patch_text}"
    );

    let retrieve = mcp_call(
        &store,
        "interop-test",
        serde_json::json!({
    ...
    assert_eq!(
        retrieve["result"]["structuredContent"]["namespace"],
        "interop-test"
    );
    ...
    assert_eq!(
        doctor["result"]["structuredContent"]["current_namespace"],
        "interop-test"
    );
}
