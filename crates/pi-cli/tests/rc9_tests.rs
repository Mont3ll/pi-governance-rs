...
fn memory_worth_scores_durable_and_secret_text() {
    let durable = assert_success(&[
        "memory-worth",
        "Going forward, always run cargo test before release.",
        "--json",
    ]);
    ...
    assert!(json["warnings"]
        ...
        .unwrap()
        .iter()
        .any(|w| w.as_str().unwrap().contains("secret")));
}
...
fn capture_creates_l2_candidate_and_daily_session_decision() {
    ...
    let captured = assert_success(&[
        ...
        "capture",
        "--text",
        "don't use echo >> for notes, use sed instead",
        "--project",
        "pi-governance-rs",
        "--json",
    ]);
    ...
    assert_success(&[
        ...
        "daily",
        "--text",
        "#decision use canonical JSONL as the source of truth",
        "--project",
        "pi-governance-rs",
        "--json",
    ]);
    let decisions = assert_success(&[
        "--store",
        &s,
        "session",
        "decisions",
...
fn inbox_context_recall_xray_and_session_workflow() {
    ...
    assert_success(&[
        ...
        "capture",
        "--text",
        "Always run cargo test and release-audit before stable tagging.",
        "--project",
        "pi-governance-rs",
    ]);
    let patches = assert_success(&["--store", &s, "list-patches"]);
    let patch_id = patches
        ...
        .find_map(|line| {
            line.split_whitespace()
                .find(|part| part.starts_with("patch_"))
        })
        .unwrap()
        .to_string();
    ...
    assert!(json["included"].as_array().unwrap()[0]
        .get("layer")
        .is_some());
...
fn mcp_lists_and_calls_rc9_tools_with_object_content() {
    ...
    let json: serde_json::Value = serde_json::from_slice(&out.stdout).unwrap();
    assert_eq!(
        json["result"]["structuredContent"]["mutation_performed"],
        false
    );
    assert_eq!(json["result"]["structuredContent"]["schema_version"], 1);
    assert!(
        json["result"]["structuredContent"]["nodes"]
            .as_array()
            .unwrap()
            .len()
            <= 200
