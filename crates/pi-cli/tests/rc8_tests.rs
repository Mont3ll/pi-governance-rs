...
fn tmp_store(name: &str) -> String {
    let p = format!("/tmp/pi-cli-{name}-{}", std::process::id());
...
fn mcp(store: &str, ns: &str, req: serde_json::Value) -> serde_json::Value {
    let mut child = Command::new(bin())
        .args(["--store", store, "--namespace", ns, "mcp-stdio"])
        .stdin(Stdio::piped())
        .stdout(Stdio::piped())
        .spawn()
        .unwrap();
...
fn mcp_inspect_record_and_maintenance_scan_work() {
    ...
    let rec = text
        .lines()
        .find(|l| l.contains("status=Active"))
        .unwrap()
        .split(']')
        .next()
        .unwrap()
    ...
    let inspected = mcp(
        &store,
        "default",
        serde_json::json!({"jsonrpc":"2.0","id":2,"method":"tools/call","params":{"name":"pi.inspect_record","arguments":{"record_id":rec}}}),
    );
    ...
    let scan = mcp(
        &store,
        "default",
        serde_json::json!({"jsonrpc":"2.0","id":3,"method":"tools/call","params":{"name":"pi.maintenance_scan","arguments":{}}}),
    );
    ...
    let missing = mcp(
        &store,
        "default",
        serde_json::json!({"jsonrpc":"2.0","id":4,"method":"tools/call","params":{"name":"pi.inspect_record","arguments":{"record_id":"rec_missing"}}}),
    );
...
fn review_actions_reject_defer_apply_and_scan_are_json() {
    ...
    assert!(!Command::new(bin())
        .args(["--store", &store, "apply", &reject_id])
        .status()
        .unwrap()
        .success());
    ...
    let out = Command::new(bin())
        .args([
            "--store", &store, "review", "--defer", &defer_id, "--reason", "QA defer", "--json",
        ])
        .output()
        .unwrap();
    assert!(
    ...
    assert!(Command::new(bin())
        .args(["--store", &store, "review", "--apply", &apply_id])
        .status()
        .unwrap()
        .success());
    let list = Command::new(bin())
    ...
    assert!(String::from_utf8(list.stdout)
        .unwrap()
        .contains("rc8 apply action memory"));

    let scan = Command::new(bin())
        .args(["--store", &store, "maintenance", "scan", "--json"])
        .output()
        .unwrap();
...
fn retriever_modes_and_redacted_export() {
    ...
    let mcp_ret = mcp(
        &store,
        "default",
        serde_json::json!({"jsonrpc":"2.0","id":5,"method":"tools/call","params":{"name":"pi.retrieve_context","arguments":{"query":"release workflow","retriever":"hybrid","explain":true}}}),
    );
    assert_eq!(
        mcp_ret["result"]["structuredContent"]["retriever"],
        "hybrid"
    );

    let export = Command::new(bin())
        .args(["--store", &store, "export", "--redacted"])
        .output()
        .unwrap();
