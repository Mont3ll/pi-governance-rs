...
fn graph_rejects_zero_limits() {
    ...
    let out = Command::new(bin())
        .args(["--store", &root, "graph", "--max-nodes", "0", "--json"])
        .output()
        .unwrap();
...
fn graph_and_quality_commands_return_versioned_read_only_json() {
    ...
    for args in [
        vec!["--store", &root, "graph", "--json"],
        vec!["--store", &root, "quality", "memory", "--json"],
        vec!["--store", &root, "quality", "relationship", "--json"],
        vec!["--store", &root, "quality", "recall", "--json"],
        vec!["--store", &root, "quality", "store", "--json"],
    ] {
...
fn workflow_intelligence_commands_are_report_only() {
    ...
    for args in [
        ...
        vec!["--store", &root, "failure-analysis", "--json"],
    ] {
        let value: serde_json::Value = serde_json::from_str(&success(&args)).unwrap();
        assert_eq!(value["mutation_performed"], false);
        assert_eq!(value["schema_version"], 1);
...
fn simulate_patch_is_read_only() {
    ...
    let patches: serde_json::Value =
        serde_json::from_str(&success(&["--store", &root, "list-patches", "--json"])).unwrap();
    let patch_id = patches
        .as_array()
        .unwrap()
        .iter()
        .find(|patch| patch["latest_status"] == "proposed")
        .unwrap()["patch_id"]
        .as_str()
        .unwrap();
    let value: serde_json::Value = serde_json::from_str(&success(&[
        "--store",
        &root,
        "simulate-patch",
        patch_id,
